...
for i in $(seq 1 "$BACKEND_COUNT"); do
    ...
    ssh -o StrictHostKeyChecking=no "backend-$i" "cd /local/repository/backend && for p in \$(seq 8000 8009); do PORT=\$p SERVER_ID=backend-$i-\$p nohup cpulimit -l 10 -- ./backend-binary > /tmp/backend_\$p.log 2>&1 & done" &
done
